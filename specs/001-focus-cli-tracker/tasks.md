...
- [X] T005 Implement data directory bootstrap and DB connection in `src/db/mod.rs`: resolve `dirs::home_dir()/.local/share/focus/`, call `std::fs::create_dir_all`, open rusqlite connection, run `PRAGMA journal_mode=WAL` (FR-018, constitution Principle V) then `CREATE TABLE IF NOT EXISTS sessions` + both indexes from `data-model.md`; map any open/parse error to `FocusError::DataFileCorrupted` with the full DB path; expose `pub fn open_db() -> anyhow::Result<Connection>` and `pub fn open_db_at(path) -> anyhow::Result<Connection>`
